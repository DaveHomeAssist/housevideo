/* Shared FMP chrome for static Pages.
 ...
 - Point legacy backfocus AV Suite links at /fmp/; orphans get FMP chrome sheets
*/
(() => {
 ...
 const inBackfocus = /\/(?:fmp\/)?backfocus(\/|$)/.test(path);
 const inOrphanRef = /^\/(switcher|shader|ursa-broadcast-g2)(\/|$)/.test(path);

 if (inFmp || inBackfocus || inOrphanRef) {
